...
CTPPSDirectProtonSimulation::CTPPSDirectProtonSimulation(const edm::ParameterSet &iConfig)
    : lhcInfoLabel_(iConfig.getParameter<std::string>("lhcInfoLabel")),
      ...
      genParticleToken_(consumes<std::vector<reco::GenParticle>>(iConfig.getUntrackedParameter<edm::InputTag>("genParticlesInputTag"))),
...
void CTPPSDirectProtonSimulation::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  TLorentzVector my_photon;
  TLorentzVector my_proton;
  TLorentzVector my_diffractedProton;
  for (const reco::GenParticle& gen : *genparticles) {
    if (fabs(gen.pdgId())==22 && (gen.pz()>-14000) && (gen.pz()<14000)){
      const Candidate * mom = gen.mother();
      if (fabs(mom->pdgId())==2212){
   //      std::cout<<"trouve "<<gen.pdgId()<<" "<<gen.pt()<<" "<<gen.eta()<<" "<<gen.phi()<<" "<<gen.pz()<<" "<<mom->pdgId()<<" "<<mom->pz()<<std::endl;
	 my_photon.SetPxPyPzE(gen.px(),gen.py(),gen.pz(),gen.energy());
	 my_proton.SetPxPyPzE(0.0,0.0,mom->pz(),mom->pz());
	 has_proton=true;
      }
    }
  }
...
//         const Candidate * mom = gen.mother();
//         const Candidate * greatmom = mom->mother();
//         if (fabs(greatmom->pdgId())==2212){
//            std::cout<<"trouve "<<gen.pdgId()<<" "<<gen.pt()<<" "<<gen.eta()<<" "<<gen.phi()<<" "<<gen.pz()<<" "<<greatmom->pdgId()<<" "<<greatmom->pz()<<std::endl;
//            my_photon.SetPtEtaPhiM(gen.pt(),gen.eta(),gen.phi(),0.0);
//            my_proton.SetPxPyPzE(0.0,0.0,greatmom->pz(),greatmom->pz());
//            has_proton=true;
//         }
//       }
//     }
//  }
  if (has_proton) my_diffractedProton=my_proton-my_photon;

  /*for (const reco::GenParticle& gen : *genparticles) {
    const Candidate * mom = gen.mother();
    if (mom != nullptr and i<100 and (i<50 or fabs(gen.pdgId())<23)) std::cout<<gen.pdgId()<<" "<<gen.pz()<<" "<<mom->pdgId()<<" "<<mom->pz()<<std::endl;
    //else if (i<50 or fabs(gen.pdgId())<23) std::cout<<gen.pdgId()<<" "<<gen.pz()<<std::endl;
    ++i;
  }*/
  ...
  for (auto it_vtx = evt->vertices_begin(); it_vtx != evt->vertices_end(); ++it_vtx) {
    ...
    for (auto it_part = vtx->particles_out_const_begin(); it_part != vtx->particles_out_const_end(); ++it_part) {
      ...
      if (part->pdg_id() != 2212)
        continue;
      //std::cout<<"avant "<<id_vtx<<" "<<part->status()<<" "<<part->momentum().pz()<<"  "<<part->pdg_id()<<std::endl;

      if (part->status() == 4 and has_proton and fabs(part->momentum().pz()-my_proton.Pz())<10){
        HepMC::FourVector* momentum=new HepMC::FourVector(part->momentum().x(),part->momentum().y(),part->momentum().z(),part->momentum().t());
        momentum->setPx(my_diffractedProton.Px());
        momentum->setPy(my_diffractedProton.Py());
        momentum->setPz(my_diffractedProton.Pz());
        momentum->setE(my_diffractedProton.E());
	const HepMC::GenParticle* part2 = new HepMC::GenParticle(*momentum, 2212, 1, part->flow(), part->polarization());
  //	std::cout<< "Pt_num "<<(my_proton.Pz()-part2->momentum().pz())<<"  "<<my_proton.Pz()<<"   "<<part2->momentum().pz()<<" Pt_den "<< my_proton.Pz()<<std::endl;
//	std::cout<<(my_proton.Pz()-part2->momentum().pz())/my_proton.Pz()<<std::endl;
        processProton(vtx,
                        part2,
                        *geometry,
                        *hLHCInfo,
