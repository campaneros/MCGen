...
#  if checkDASInput.checkDASInput(inputdataset,step,campaign):
 #   sys.exit()
  ...
  os.system("sed -i 's|###INPUTDATASETTAG###|"+campaign+"_"+step+"-v2|g' "+crabwd+"/submit_crab.py")
#  os.system("sed -i 's|###UNITSPERJOB###|"+str(int(nevents)/int(nsplitjobs))+"|g' "+datasetname+"/submit_crab.py")
  ...
  cmsdriver_l = "cmsDriver.py step1 --no_exec --mc --python_filename run_crab.py --fileout "+step+".root --eventcontent AODSIM --runUnscheduled --datatier AODSIM --geometry DB:Extended -n 6284 "+add_cmsdriver 
